...
# 兜底检测配置
SAFEGUARD_THRESHOLD=33
SAFEGUARD_MIN_DURATION=5
taskset -p 2 $$
...
send_fcm_heartbeat() {
    ...
    local is_safeguard="$3"  # 新增：是否为兜底触发
...
fcm_loop() {
    ...
    local prev_prev_duration=0  # 上上次，用于确认模式
    local consecutive_similar=0  # 连续相似次数
...
EOF
            ...
            prev_prev_duration=$prev_duration
        ...
        # 场景3：兜底检测 - 连接重置检测
            ...
            # 计算与上次的差值（绝对值）
            local duration_diff=$((conn_duration - prev_duration))
            [ "$duration_diff" -lt 0 ] && duration_diff=$((-duration_diff))
            
            # 如果差值在阈值内（±10秒），说明连接被重置了
            if [ "$duration_diff" -le "$SAFEGUARD_THRESHOLD" ]; then
                consecutive_similar=$((consecutive_similar + 1))
                
                # 连续两次相似，触发兜底
                if [ "$consecutive_similar" -ge 2 ]; then
                    need_heartbeat=true
                    heartbeat_reason="兜底检测[连接重置 suspected: ${prev_prev_duration}s -> ${prev_duration}s -> ${conn_duration}s, diff=${duration_diff}s]"
                    is_safeguard="true"
                    consecutive_similar=0  # 重置
                fi
            else
                # 差值正常，重置计数器
...
echo "兜底检测: 连续两次时长变化≤${SAFEGUARD_THRESHOLD}秒且≥${SAFEGUARD_MIN_DURATION}秒时触发"
fcm_loop &
PID_FCM=$!
