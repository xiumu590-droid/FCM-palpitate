...
get_fcm_details() {
    ...
    echo "$ip_port|$last_ping"  # 改为 | 分隔符
}

# 2. 修复 check_fcm_connection 函数 - 所有 echo 输出改用 | 分隔符
check_fcm_connection() {
    ...
    if [ -z "$heartbeat_line" ]; then
        ...
        echo "已连接|$duration|$ip_port|$last_ping"  # 改为 | 分隔符
        return 0
    fi
    ...
    if [ -z "$total_seconds" ] || [ -z "$remaining_time" ]; then
        echo "未知|0|$ip_port|$last_ping"  # 改为 | 分隔符
        return 1
    fi
    ...
    echo "已连接|$connected_duration|$ip_port|$last_ping"  # 改为 | 分隔符
    return 0
}

# 3. 修复 fcm_loop 函数中的解析部分 - 所有 cut -d: 改为 cut -d'|'
fcm_loop() {
    sleep 2
    ...
    while true; do
        ...
        if [ "$is_first_run" = true ]; then
            ...
            local ip_port=$(echo "$details" | cut -d'|' -f1)  # 改为 |
            local ping=$(echo "$details" | cut -d'|' -f2)      # 改为 |
...
EOF
            ...
            prev_duration=$(echo "$conn_result" | cut -d'|' -f2)  # 改为 |
        ...
        local conn_result=$(check_fcm_connection)
        local conn_status=$(echo "$conn_result" | cut -d'|' -f1)      # 改为 |
        local conn_duration=$(echo "$conn_result" | cut -d'|' -f2)    # 改为 |
        local conn_ip=$(echo "$conn_result" | cut -d'|' -f3)          # 改为 |
        local conn_ping=$(echo "$conn_result" | cut -d'|' -f4)        # 改为 |
        ...
        if [ "$need_heartbeat" = true ]; then
            ...
            sleep 1
            local details=$(get_fcm_details)
            local new_ip=$(echo "$details" | cut -d'|' -f1)      # 改为 |
            local new_ping=$(echo "$details" | cut -d'|' -f2)    # 改为 |
...
echo "[$(date '+%F %T')] 启动FCM心跳守护进程..."
echo "兜底检测: 上次-这次 在 [${SAFEGUARD_MIN_DIFF}, ${SAFEGUARD_MAX_DIFF}] 范围内时触发"
echo "检查间隔: ${FCM_CHECK_INTERVAL}秒, 强制心跳阈值: ${FCM_MAX_CONNECTION_DURATION}秒"
echo ""

fcm_loop &
PID_FCM=$!
echo "智能FCM监控循环已启动 (PID: $PID_FCM)"
echo "脚本运行中，按 Ctrl+C 或执行 'kill $PID_FCM' 停止"
echo ""
wait
